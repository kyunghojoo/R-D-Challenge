...
public class UpperLowerGraph {
	...
	public UpperLowerGraph(JFrame fr, int x, int y, int width, int height, int draw_interval)
	{
		this.innerframe = fr;
		this.width = width;
		this.height = height;
	...
	private void drawChart(ChartViewer viewer)
    {
        ...
        c.setClipping();
        c.addTitle("Bus Load Monitor", "Times New Roman Bold Italic", 15).setBackground(0xdddddd, 0x000000, Chart.glassEffect());

        LegendBox b = c.addLegend2(55, 33, 3, "Arial Bold", 9);
        b.setBackground(Chart.Transparent, Chart.Transparent);
        b.setWidth(this.width-100);

        c.yAxis().setTitle("CAN Bus Load (%)", "Arial Bold", 13);
        ...
        if (lastTime != null)
        {
            ...
            LineLayer datalayer = c.addLineLayer(dataSeries, 0x800080, "Measured Bus Load");
            datalayer.setXData(timeStamps);
            datalayer.setLineWidth(2);           

            LineLayer lineLayer = c.addLineLayer2();
            lineLayer.addDataSet(UpperY, 0x338033, "Normal Zone");
