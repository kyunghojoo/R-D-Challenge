...
public class DataObject implements Serializable {
	...
	public DataObject(DataObject input)
	{
		this.socket_ts = input.socket_ts;
		this.ts = input.ts;
		this.id = input.id;
		this.type = input.type;
		this.data_length = input.data_length;
		this.data = input.data.clone();
	}
	...
	{
		return "TS : " + ts + " // ID : " + String.format("%04X", this.id) + " // Type : " + String.format("%03X", this.type) + " // DLC : " + this.data_length + " // Data : " + BytesUtils.HextoString(this.data, 0, this.data.length);
	}
